...
const faceDetect = (input) => new Promise((resolve, reject) => {
...
const getBoost = async (buffer, frameNum = 0) => {
  ...
  return faceDetect(reduceQualityBuffer).catch(() => []) || [];
};

const execute = async (bufferTemp) => {
  let buffer = bufferTemp;
  ...
  const imageAlreadyHasBorder = hasBorder || isTransparent;

  let boost = await getBoost(buffer, 0);
  if ((!boost || boost.length <= 0) && isGif) {
    boost = await getBoost(buffer, 1);
  }
